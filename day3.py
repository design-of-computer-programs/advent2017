...
def generate_sequence(N, initial_position=(0, 0), initial_value=1): 
    ...
    for i in range(N):
        if i != 0: 
            next_step = generate_next_step(*position, already_position=already_position)
...
def generate_next_step(x, y, already_position=set()): 
    if y == 0 and x > 0: step = x, y + 1
    elif y == 0 and x < 0: step = x, y - 1
    elif x == 0 and y > 0: step = x - 1, y
    elif x == 0 and y < 0: step = x + 1, y
    else:
        ...
        min_distance = min(map(lambda step: distance(*step), next_possible_step))
        for step in next_possible_step: 
